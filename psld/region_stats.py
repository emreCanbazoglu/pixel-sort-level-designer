...
def validate_grid_regions(
    ...
    *,
    ...
    max_largest_region: int | None = None,
    max_color_share: float | None = None,
    ...
    if max_largest_region is not None:
        for cs in stats.colors:
            if cs.largest > max_largest_region:
                reasons.append(
                    f"color {cs.color_index}: largest_region {cs.largest} > max_largest_region {max_largest_region}"
                )

    if max_color_share is not None:
        if not (0.0 < max_color_share <= 1.0):
            raise ValueError("max_color_share must be in (0, 1]")
        if stats.occupied_cells > 0:
            for cs in stats.colors:
                share = cs.total_cells / stats.occupied_cells
                if share > max_color_share:
                    reasons.append(
                        f"color {cs.color_index}: share {share:.4f} > max_color_share {max_color_share:.4f}"
                    )
